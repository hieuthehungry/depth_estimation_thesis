...
import torch.nn as nn
import torch
from .obj_and_rel import REL_CLASSES, CLASSES
...
ALL_TOKENS = REL_CLASSES + CLASSES
CLS_OFFSET = len(REL_CLASSES)

class SharedTextEncoder(nn.Module):
    def __init__(self, all_tokens, encoder_type='clip', pretrained_model='openai/clip-vit-base-patch16', out_dim=256):
        super().__init__()
        ...
        if encoder_type == 'bert':
            ...
            self.linear = nn.Linear(self.encoder.config.hidden_size, out_dim)

            encoded = self.tokenizer(all_tokens, padding=True, return_tensors='pt', truncation=True)
            self.register_buffer("input_ids", encoded['input_ids'])
            self.register_buffer("attention_mask", encoded['attention_mask'])
            with torch.no_grad():
                outputs = self.encoder(input_ids=encoded['input_ids'], attention_mask=encoded['attention_mask'])
                self.register_buffer("cls_embed", outputs.last_hidden_state[:, 0, :])

        elif encoder_type == 'clip':
            self.tokenizer = CLIPTokenizer.from_pretrained(pretrained_model)
            self.encoder = CLIPTextModel.from_pretrained(pretrained_model)
            for p in self.encoder.parameters():
                p.requires_grad = False
            self.linear = nn.Sequential(
                nn.Linear(self.encoder.config.hidden_size, out_dim),
                nn.GELU()
            )
            tokens = self.tokenizer(all_tokens, padding=True, return_tensors='pt')
            self.register_buffer("input_ids", tokens["input_ids"])
            self.register_buffer("attention_mask", tokens["attention_mask"])
    ...
    def forward(self, token_ids):
        ...
        return self.linear(cls_embed[token_ids.to(cls_embed.device)])


class SceneGraphEncoder(nn.Module):
    def __init__(self, node_dim, out_dim, rel_classes=REL_CLASSES):
        super().__init__()
        self.embed_rel = SharedTextEncoder(ALL_TOKENS, out_dim=out_dim)
        self.gat = GATConv(out_dim, out_dim, edge_dim=out_dim)

    def forward(self, sub_logits, obj_logits, rel_logits, sub_boxes, obj_boxes, pred_boxes):
        B, T, C = sub_logits.shape

        sub_probs = sub_logits[:, :, :-1].softmax(dim=-1)
        obj_probs = obj_logits[:, :, :-1].softmax(dim=-1)

        sub_max_vals, sub_ids = sub_probs.max(dim=-1)
        obj_max_vals, obj_ids = obj_probs.max(dim=-1)
        edge_indices, edge_types = extract_scene_graph_edges(
                sub_boxes,
                obj_boxes,
                rel_logits,
                pred_boxes
