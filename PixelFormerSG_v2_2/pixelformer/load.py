...
def eval(model, dataloader_eval, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        print(eval_sample_batched['path'], measures)
...
def main_worker(args):
    ...
    if args.checkpoint_path != '':
        if os.path.isfile(args.checkpoint_path):
            ...
            for k, v in checkpoint['model'].items():
                ...
                name = name.replace("proj_x", "proj_e" )
                name = name.replace("proj_v", "proj_q" )
                name = name.replace("bin_mlp", "bcp" )
    ...
    torch.save(checkpoint, './pretrained/nyu.pth')
