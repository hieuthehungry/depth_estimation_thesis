...
class PixelFormerSG(nn.Module):
    ...
    def forward(self, imgs, scene_graph):
        ...
        sg_feat_q1 = []
        for feat in sg_feat_q1_list:
            if feat.numel() == 0:  # Skip empty features
                sg_feat_q1.append(torch.zeros(feat.shape[1], device=feat.device))
            else:
                sg_feat_q1.append(feat.mean(dim=0))

        sg_feat_q1 = torch.stack(sg_feat_q1, dim=0).unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
