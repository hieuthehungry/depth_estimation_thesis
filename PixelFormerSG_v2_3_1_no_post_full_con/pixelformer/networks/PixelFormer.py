...
class PixelFormerSG(nn.Module):
    ...
    def sg_mean_pooling(self, sg_feats_list):
        sg_feat = []
        for feat in sg_feats_list:
            if feat.numel() == 0:  # Skip empty features
                sg_feat.append(torch.zeros(feat.shape[1], device=feat.device))
            else:
                sg_feat.append(feat.mean(dim=0))
        sg_feat = torch.stack(sg_feat, dim=0).unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
    ...
    def forward(self, imgs, scene_graph):
        ...
        sg_feat_q4 = self.sg_mean_pooling(sg_feat_q4_list)

        sg_feat_q3_list = self.sg_encoder_q3(enc_feats[2], graph_data_list)
        sg_feat_q3 = self.sg_mean_pooling(sg_feat_q3_list)

        sg_feat_q2_list = self.sg_encoder_q2(enc_feats[1], graph_data_list)
        sg_feat_q2 = self.sg_mean_pooling(sg_feat_q2_list)

        sg_feat_q1_list = self.sg_encoder_q1(enc_feats[0], graph_data_list)
        sg_feat_q1 = self.sg_mean_pooling(sg_feat_q1_list)
     

        q3 = self.sam4(enc_feats[3], q4 + sg_feat_q4)
        q3 = nn.PixelShuffle(2)(q3)
      
        q2 = self.sam3(enc_feats[2], q3 + sg_feat_q3)
        q2 = nn.PixelShuffle(2)(q2)


        q1 = self.sam2(enc_feats[1], q2 + sg_feat_q2)
        q1 = nn.PixelShuffle(2)(q1)


        q0 = self.sam1(enc_feats[0], q1 + sg_feat_q1)
