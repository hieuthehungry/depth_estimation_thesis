...
from networks.LightDepth import LightDepth


parser = argparse.ArgumentParser(description='LightDepth PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--mode',                      type=str,   help='train or test', default='train')
parser.add_argument('--model_name',                type=str,   help='model name', default='lightdepth')
parser.add_argument("--encoder",                   type=str,   help='encoder name', default='mobilenetv3_small_100')
parser.add_argument('--yolo_weights',              type=str,   help='path of pretrained yolo', default=None)
parser.add_argument('--fusion_method',             type=str,   help='fusion method, perceiver or mlp', default= "perceiver")
parser.add_argument('--decoder_channels',          type=int,   help='number of FastDepth Decoder channel', default= 256)
parser.add_argument('--num_classes',               type=int,   help='number of classes', default= 80)
parser.add_argument('--node_dim',                  type=int,   help='scene graph node dimension', default= 256)
parser.add_argument('--relation_dim',              type=int,   help='Relation vector dimension', default= 128)
parser.add_argument('--max_objects',               type=int,   help='max_objects per image', default= 16)
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        with torch.no_grad():
            ...
            if not has_valid_depth:
                ...
                continue
            start_time = time()
            pred_depth, init_depth, bboxes = model(image)
            if post_process:
                # --- During evaluation:
                # Flip image
                ...
                # Post-process
                pred_depth, init_depth, bboxes = post_process_depth(pred_depth, pred_depth_flipped)
            end_time = time()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = LightDepth(encoder = args.encoder, yolo_weights=args.yolo_weights, fusion_method= args.fusion_method, 
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            depth_est, init_depth, bboxes = model(image)
...
def main():
    ...
    if save_files:
        ...
        command = 'cp light_depth/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp light_depth/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp light_depth/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
