...
class CrossAttentionFusion(nn.Module):
    def __init__(self, d_model, num_heads=8):
        ...
        self.norm_q4 = nn.LayerNorm(d_model)
        self.norm_nodes = nn.LayerNorm(d_model)

    def forward(self, q4_feat, sg_nodes):
        B, C, H, W = q4_feat.shape
        q4_flat = q4_feat.flatten(2).transpose(1, 2)  # [B, HW, C]
        sg_nodes = self.norm_nodes(sg_nodes)
        q4_flat = self.norm_q4(q4_flat)

        # Let q4 features attend to scene graph nodes
        fused, _ = self.attn(q4_flat, sg_nodes, sg_nodes)  # Q, K, V
        fused = fused.transpose(1, 2).view(B, C, H, W)

        return q4_feat + fused  # residual
...
class PixelFormerSG(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        self.cross_fusion = CrossAttentionFusion(d_model=v_dims[3], num_heads=8)
    ...
    def forward(self, imgs, scene_graph):
        ...
        # Cross-attn
        max_nodes = max(feat.shape[0] for feat in sg_feat_q4_list)
        padded_nodes = []
        for feat in sg_feat_q4_list:
            if feat.shape[0] < max_nodes:
                pad = torch.zeros((max_nodes - feat.shape[0], feat.shape[1]), device=feat.device)
                feat = torch.cat([feat, pad], dim=0)
            padded_nodes.append(feat.unsqueeze(0))
        sg_feat_q4 = torch.cat(padded_nodes, dim=0)  # [B, N, C]
        q4 = self.cross_fusion(q4, sg_feat_q4)
