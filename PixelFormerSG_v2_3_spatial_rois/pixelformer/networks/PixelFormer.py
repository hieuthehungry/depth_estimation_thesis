...
class SceneGraphEncoder(nn.Module):
    ...
    def forward(self, feat_map, sg_data_list):
        ...
        for b in range(B):
            rois = self.xywh_to_xyxy(sg_data_list[b]["node_boxes"])
            rois[:, 0::2] *= W
            rois[:, 1::2] *= H

            num_boxes = rois.size(0)
            batch_idx = torch.zeros((num_boxes, 1), device=feat_map.device)
            roi_boxes = torch.cat([batch_idx, rois], dim=1)

            roi_feats = roi_align(
                feat_map[b].unsqueeze(0), roi_boxes,
                output_size=self.roi_size, spatial_scale=1.0, aligned=True
            )  # [N, C, h', w']
            pooled_feats = roi_feats.mean(dim=[2, 3])  # [N, C]
            node_feats = self.node_proj(roi_feats).mean(dim=[2, 3])
            ...
            edge_attr = self.edge_proj(edge_attr_input)
